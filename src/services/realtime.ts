  ...
  pedidosChannel = supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        // Notificar especificamente sobre liberação
        if ((payload.new as any).liberado_pelo_estabelecimento === true) {
          console.log('🔓 [REALTIME] Pedido liberado pelo estabelecimento:', payload.new.id);
        }
      }
    )
    .subscribe((status) => {
...
/**
 * Assinar liberação de pedidos em tempo real (específico)
 */
export const assinarLiberacaoPedidos = (
  onPedidoLiberado?: (pedido: Pedido) => void
) => {
  if (!pedidosChannel) {
    console.log('📡 Criando canal para liberação de pedidos...');
  }

  const channel = supabase
    .channel('pedidos-liberacao')
    .on(
      'postgres_changes',
      {
        event: 'UPDATE',
      ...
      (payload) => {
        // Verificar se é uma liberação
        if ((payload.new as any).liberado_pelo_estabelecimento === true &&
            (payload.old as any).liberado_pelo_estabelecimento === false) {
          console.log('🔓 [REALTIME] Pedido liberado:', payload.new);
          onPedidoLiberado?.(payload.new as Pedido);
        }
      }
    )
    .subscribe();
